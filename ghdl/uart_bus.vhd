...
entity uart_bus is
  ...
  port (
    ...
    btnD : in  std_logic;               --reset (active high)
    RsRx : in  std_logic;
    RsTx : out std_logic;
    clk  : in  std_logic);
...
architecture arch of uart_bus is
  ...
  constant CLK_HZ : integer := 50000000; -- 50MHz clock
  constant BAUD : integer := 115200;    -- test bit rate
...
--  RsTx  <= RsRx;                        -- direct echo
  ...
  uart_new_2: entity work.uart_new
    generic map (
      SYSTEM_CLK_HZ => CLK_HZ,
      BAUD_RATE     => BAUD)
    port map (
      rst       => btnD,
      RsRx      => RsRx,
